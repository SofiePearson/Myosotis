...
#MycpDNA <- read.nexus(file = "All_My_n259_plastome_final.treefile")
MycpDNA <- read.tree(file = "All_My_n259_plastome_reroot.treefile")

T0 = ggtree(MycpDNA, branch.length = "none") +  
...
mycha <- read.csv("Myosotis_characters_by_individuals2024NEW.csv", header = T,  na.strings=c("NA"," ")) %>% 
  filter(individual %in% MycpDNA$tip.label)
...
pdf("Myso_cpDNA_fivebars2_withname.pdf", width = 15, height = 22)
T5
dev.off()


T6 <- collapse(T5, 498, 'mixed', fill='grey70', alpha=.4) %>% 
  collapse(430, 'mixed', fill='grey70', alpha=.4) %>%
  collapse(423, 'mixed', fill='grey70', alpha=.4) %>%
  collapse(404, 'mixed', fill='grey70', alpha=.4) %>%
  collapse(355, 'mixed', fill='grey70', alpha=.4) %>%
  collapse(351, 'mixed', fill='grey70', alpha=.4) %>%
  collapse(347, 'mixed', fill='grey70', alpha=.4) %>%
  collapse(341, 'mixed', fill='grey70', alpha=.4) %>%
  collapse(318, 'mixed', fill='grey70', alpha=.4) %>%
  collapse(301, 'mixed', fill='grey70', alpha=.4) %>%
  collapse(271, 'mixed', fill='grey70', alpha=.4) 


T6

pdf("Myso_cpDNA_fivebars2.pdf", width = 15, height = 22)
