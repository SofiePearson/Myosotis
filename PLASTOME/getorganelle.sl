...
#########################
## mapping to reference. example from one plate
#########################
#!/bin/bash -e
#SBATCH --account		montpt03477
...
for file in /nesi/nobackup/montpt03477/Weixuan/plastome/01_trimmed/CHL_P1_0*_1P.fq
	do 
	withpath="${file}" 
	filename=${withpath##*/} 
	base="${filename%*_1P.fq}" 
	...
	get_organelle_from_reads.py --genes  reference_plastome.fasta -1 ../01_trimmed/"${base}"_1P.fq -2 ../01_trimmed/"${base}"_2P.fq -o plastome_output/"${base}" -R 40 -t 60 -w 60 -k 21,45,65,85 -F embplant_pt --max-reads inf --reduce-reads-for-coverage inf --max-extending-len 6000 --overwrite
